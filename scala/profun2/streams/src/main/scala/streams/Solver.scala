...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case head #:: tail =>
      ...
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(head._1, head._2), newExplored)
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from((startBlock, List.empty) #:: Stream.empty, Set())
