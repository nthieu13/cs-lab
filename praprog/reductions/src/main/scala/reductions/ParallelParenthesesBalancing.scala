...
object ParallelParenthesesBalancing {
  ...
  def balance(chars: Array[Char]): Boolean = {
    def balanceRecur(chars: Array[Char], depth: Int): Boolean = {
      if (chars.isEmpty) {
        if (depth == 0) true
        else false
      } else {
        if (chars.head == '(') balanceRecur(chars.tail, depth + 1)
        else if (chars.head == ')') {
          if (depth == 0) false
          else balanceRecur(chars.tail, depth - 1)
        }
        else balanceRecur(chars.tail, depth)
      }
    }
