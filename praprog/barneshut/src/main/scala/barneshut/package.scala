...
package object barneshut {
  ...
  extends Quad {
    ...
    val mX = bodies.map(b => b.x * b.mass).sum / m
    val mY = bodies.map(b => b.y * b.mass).sum / m
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val minX = math.min(math.max(boundaries.minX, b.x), boundaries.maxX)
      val minY = math.min(math.max(boundaries.minY, b.y), boundaries.maxY)
      val x = ((minX - boundaries.minX) / sectorSize).toInt
      val y = ((minY - boundaries.minY) / sectorSize).toInt
      apply(x, y) += b
      this
    }
